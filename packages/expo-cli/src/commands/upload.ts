    ...
    try {
      if (process.platform !== 'darwin') {
        if (platform === 'android') {
          ...
          log(
            chalk.bold(
              'Try --use-submission-service flag to your app from Expo servers. This feature is still experimental!'
            )
          );
        } else {
...
async function submitAndroidApp(ctx: SubmissionContext): Promise<void> {
  ...
  const submitter = new AndroidSubmitter(submissionOptions);
...
function resolveArchiveSource(ctx: SubmissionContext): Result<ArchiveSource> {
  ...
  if (ctx.options.url) {
    ...
    const exp = getExpoConfig(ctx.projectDir);
    if (!exp.slug) {
      return result(new Error('expo.slug is not defined in app.json'));
    }
    return result({
